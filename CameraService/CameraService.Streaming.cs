...
namespace CameraService
{
    public partial class CameraService
    {
        ...
        public async Task StartVideoStream(CancellationToken cancellationToken)
        {
            ...
            var arguments =
                "-c \"raspivid -o - -t 0 -w 800 -h 600 -fps 12 --nopreview  | cvlc -vvv stream:///dev/stdin --sout '#rtp{sdp = rtsp://:80/}' :demux=h264\"";
            Process process = new();
            ...
            process.Exited += (sender, args) => { _logger.LogInformation("Cancelling video stream..."); };
        }
    }
}